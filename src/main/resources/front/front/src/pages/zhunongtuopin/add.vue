...
<div :style='{"width":"80%","padding":"20px 0","margin":"0 auto","position":"relative","background":"#FCFAFF"}'>
    ...
    >
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="文章编号" prop="wenzhangbianhao">
              <el-input v-model="ruleForm.wenzhangbianhao" placeholder="文章编号" disabled></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="文章标题" prop="wenzhangbiaoti">
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}'  label="文章分类" prop="wenzhangfenlei">
            <el-select v-model="ruleForm.wenzhangfenlei" placeholder="请选择文章分类" :disabled=" false  ||ro.wenzhangfenlei" >
              <el-option
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="文章照片" v-if="type!='cross' || (type=='cross' && !ro.wenzhangzhaopian)" prop="wenzhangzhaopian">
            <file-upload
            tip="点击上传文章照片"
            action="file/upload"
            :limit="3"
            :multiple="true"
            :fileUrls="ruleForm.wenzhangzhaopian?ruleForm.wenzhangzhaopian:''"
            @change="wenzhangzhaopianUploadChange"
            ></file-upload>
          </el-form-item>
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="文章简介" prop="wenzhangjianjie">
            <el-input
              type="textarea"
              :rows="8"
              placeholder="文章简介"
              v-model="ruleForm.wenzhangjianjie">
            </el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="文章详情" prop="wenzhangxiangqing">
            <editor 
...
<script>
  export default {
      ...
      init(type) {
        ...
        if(type=='cross'){
          ...
          for (var o in obj){
            ...
            if(o=='wenzhangzhaopian'){
              this.ruleForm.wenzhangzhaopian = obj[o].split(",")[0];
              this.ro.wenzhangzhaopian = true;
              continue;
            }
            if(o=='fabushijian'){
            ...
            if(o=='wenzhangjianjie'){
              this.ruleForm.wenzhangjianjie = obj[o];
              this.ro.wenzhangjianjie = true;
              continue;
            }
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`zhunongtuopin/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
      ...
      onSubmit() {
			...
			if(this.ruleForm.wenzhangbianhao){
				this.ruleForm.wenzhangbianhao = String(this.ruleForm.wenzhangbianhao)
			}
			//更新跨表属性
			var crossuserid;
			var crossrefid;
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('zhunongtuopin/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`zhunongtuopin/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
      ...
      wenzhangzhaopianUploadChange(fileUrls) {
          this.ruleForm.wenzhangzhaopian = fileUrls.replace(new RegExp(this.$config.baseUrl,"g"),"");;
      },
    }
  };
</script>
